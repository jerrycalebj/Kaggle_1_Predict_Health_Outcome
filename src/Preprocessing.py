from My_Libraries import pd, np, RobustScaler, MinMaxScaler


cols_to_round = ['age','height(cm)', 'weight(kg)']
def round_columns_to_nearest_multiple_of_5(df, cols_to_round):
    new_df = df.copy()
...
def process_data(df):
    df_func = df.copy()
    df_func = round_columns_to_nearest_multiple_of_5(df_func, cols_to_round)
